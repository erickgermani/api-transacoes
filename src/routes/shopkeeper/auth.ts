...
const validate = async (req: Request, res: Response, next: NextFunction) => {
	try {
		await shopkeeperService.validate(req.body);
		next();
	} catch (err) {
		next(err);
	}
...
router.post('/signup', validate, async (req: Request, res: Response, next: NextFunction) => {
	try {
		const result = await shopkeeperService.save(req.body);
...
router.post('/signin', async (req: Request, res: Response, next: NextFunction) => {
	try {
		const user = await shopkeeperService.findOne({ mail: req.body.mail });
